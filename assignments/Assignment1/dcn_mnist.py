...
def main():
    ...
    def summary_all(var, var_name):
        tf.summary.scalar(var_name + '_mean', tf.reduce_mean(var))
        tf.summary.scalar(var_name + '_max', tf.reduce_max(var))
        tf.summary.scalar(var_name + '_min', tf.reduce_min(var))
        tf.summary.scalar(var_name + '_std', (tf.reduce_sum(var - tf.reduce_mean(var)))**2)
        tf.summary.histogram(var_name + '_hist', var)

    tf.summary.scalar(cross_entropy.op.name, cross_entropy)
    summary_all(W_conv1, 'conv1_weight')
    summary_all(b_conv1, 'conv1_bias')
    summary_all(x_image, 'conv1_input')
    summary_all(h_conv1, 'conv1_activation')    
    summary_all(W_conv2, 'conv2_weight')
    summary_all(b_conv2, 'conv2_bias')
    summary_all(h_pool1, 'conv2_input')
    summary_all(h_conv2, 'conv2_activation')    
    summary_all(W_fc1, 'fc_weight')
    summary_all(b_fc1, 'fc_bias')
    summary_all(h_pool2_flat, 'fc_input')
    summary_all(W_fc2, 'softmax_weight')
    summary_all(b_fc2, 'softmax_bias')
    summary_all(h_fc1_drop, 'softmax_input')
    ...
    for i in range(max_step):
        ...
        if i % 1100 == 0 or i == max_step:
            summary_str = sess.run(summary_accuracy, feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5})
            summary_writer.add_summary(summary_str, i)
