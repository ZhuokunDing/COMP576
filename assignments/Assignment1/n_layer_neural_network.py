...
class Layer():
    '''
    This class build a single layer of a specific input dim and output dim
    '''
...
class DeepNeuralNetwrok(three_layer_neural_network.NeuralNetwork):
    ...
    def __init__(self, n_hidden, input_dim, hidden_dim, output_dim, actFun_type='tanh', reg_lambda=0.01, seed=0):
        ...
        self.layers = [Layer(self.input_dim, self.hidden_dim, with_type(self.actFun, actFun_type), with_type(self.diff_actFun, actFun_type))]
        for _ in range(self.n_hidden):
            self.layers += [Layer(hidden_dim, hidden_dim, with_type(self.actFun, actFun_type), with_type(self.diff_actFun, actFun_type))]
